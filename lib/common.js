	...
	constructor(type) {
		const typeData = {
			...
			"pullUp": {
				...
				to: 0,
				duration: 200,
				requiredStatus: ["pullDownFinish"],
				endStatus: "init"

			}
		};
		this.type = type;
		this.property = typeData[type].property;
		this.unit = typeData[type].unit;
		this.from = typeData[type].from;
		this.to = typeData[type].to;
		this.dist = this.to - this.from;
		this.duration = typeData[type].duration;
		this.requiredStatus = typeData[type].requiredStatus;
		this.endStatus = typeData[type].endStatus;
...
class Animation {
	constructor(element) {
		...
		element.animation = {};
		element.animation.status = "init";
		element.animate = function (type, fun) {
			element.animationData = new AnimationData(type);
			if (!matchStatus(element.animationData.requiredStatus))
				return;
			let { property, from, dist, unit, duration } = element.animationData;
